...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{\textbackslash{}newtheorem\{theorem\}\{Theorem\}}
\NormalTok{\textbackslash{}newtheorem\{lemma\}[theorem]\{Lemma\}}
\end{Highlighting}
\end{Shaded}
...
\protect\hypertarget{ex:unnamed-chunk-10}{}{\label{ex:unnamed-chunk-10}}Derivamos
la función característica de \(X\sim U(0,1)\) con la función de densidad
de probabilidad \(f(x)=\mathbf{1}_{x \in [0,1]}\).
...
\protect\hypertarget{prp:unnamed-chunk-11}{}{\label{prp:unnamed-chunk-11}}La
distribución de la suma de variables aleatorias independientes Poisson
\(X_i \sim \mathrm{Pois}(\lambda_i),\: i=1,2,\cdots,n\) es
...
\begin{figure}
\includegraphics[width=0.5\linewidth]{bookdown_files/figure-latex/multi-plots2-1} \includegraphics[width=0.5\linewidth]{bookdown_files/figure-latex/multi-plots2-2} \caption{Dos gráficos dispuestos uno al lado del otro.}\label{fig:multi-plots2}
\end{figure}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{build_chapter =}\StringTok{ }\NormalTok{function(}
...
\NormalTok{\textbackslash{}usepackage\{longtable\}}

\NormalTok{\textbackslash{}ifxetex}
  \NormalTok{\textbackslash{}usepackage\{letltxmacro\}}
  \NormalTok{\textbackslash{}setlength\{\textbackslash{}XeTeXLinkMargin\}\{1pt\}}
  \NormalTok{\textbackslash{}LetLtxMacro\textbackslash{}SavedIncludeGraphics\textbackslash{}includegraphics}
  \NormalTok{\textbackslash{}def\textbackslash{}includegraphics#1#\{}\CommentTok{% #1 catches optional stuff (star/opt. arg.)}
    \NormalTok{\textbackslash{}IncludeGraphicsAux\{#1\}}\CommentTok{%}
  \NormalTok{\}}\CommentTok{%}
  \NormalTok{\textbackslash{}newcommand*\{\textbackslash{}IncludeGraphicsAux\}[2]\{}\CommentTok{%}
    \NormalTok{\textbackslash{}XeTeXLinkBox\{}\CommentTok{%}
      \NormalTok{\textbackslash{}SavedIncludeGraphics#1\{#2\}}\CommentTok{%}
    \NormalTok{\}}\CommentTok{%}
  \NormalTok{\}}\CommentTok{%}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{book_filename:} \StringTok{"mi-libro.Rmd"}
\FunctionTok{chapter_name:} \StringTok{"CAPÍTULO "}
\FunctionTok{before_chapter_script:} \KeywordTok{[}\StringTok{"script1.R"}\KeywordTok{,} \StringTok{"script2.R"}\KeywordTok{]}
\FunctionTok{after_chapter_script:} \StringTok{"script3.R"}
\FunctionTok{edit:}
  \FunctionTok{link:} \NormalTok{https://github.com/rstudio/bookdown/edit/master/inst/examples/%s}
  \FunctionTok{text:} \StringTok{"Edit"}
\FunctionTok{output_dir:} \StringTok{"book-output"}
\FunctionTok{clean:} \KeywordTok{[}\StringTok{"my-book.bbl"}\KeywordTok{,} \StringTok{"R-packages.bib"}\KeywordTok{]}
\end{Highlighting}
\end{Shaded}
...
versión preliminar de RStudio}\index{RStudio IDE} si su versión RStudio
...
\OperatorTok{\}}\NormalTok{)()}\OperatorTok{;}
\OperatorTok{<}\SpecialStringTok{/script>}
\SpecialStringTok{<noscript>Please enable JavaScript to view the}
\SpecialStringTok{<a href="https://disqus.com/}\OperatorTok{?}\NormalTok{ref_noscript}\StringTok{">}
  \NormalTok{comments powered by }\VariableTok{Disqus}\NormalTok{.}\OperatorTok{<}\SpecialStringTok{/a></noscript}\OperatorTok{>}
\end{Highlighting}
\end{Shaded}
...
\texttt{after\_body} (Figura @ref(fig: disqus) muestra cómo se ve el
área de discusión):
...
\FunctionTok{output:}
  \FunctionTok{bookdown:}\NormalTok{:gitbook:}
    \FunctionTok{includes:}
      \FunctionTok{after_body:} \NormalTok{disqus.html}
\OtherTok{---}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{git} \NormalTok{config --global user.email }\StringTok{"you@example.com"}
\KeywordTok{git} \NormalTok{config --global user.name }\StringTok{"Your Name"}

\CommentTok{# clone the repository to the book-output directory}
\KeywordTok{git} \NormalTok{clone -b gh-pages \textbackslash{}}
  \NormalTok{https://}\OtherTok{$\{GITHUB_PAT\}}\NormalTok{@github.com/}\OtherTok{$\{TRAVIS_REPO_SLUG\}}\NormalTok{.git \textbackslash{}}
  \NormalTok{book-output}
\KeywordTok{cd} \NormalTok{book-output}
\KeywordTok{cp} \NormalTok{-r ../_book/* ./}
\KeywordTok{git} \NormalTok{add --all *}
\KeywordTok{git} \NormalTok{commit -m}\StringTok{"Update the book"}
\KeywordTok{git} \NormalTok{push origin gh-pages}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{sudo:} \NormalTok{false}

\FunctionTok{cache:}
  \FunctionTok{packages:} \NormalTok{yes}
...
\NormalTok{\textbackslash{}renewcommand\{\textbackslash{}href\}[2]\{#2\textbackslash{}footnote\{\textbackslash{}url\{#1\}\}\}}
\end{Highlighting}
\end{Shaded}
